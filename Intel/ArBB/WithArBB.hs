...
----------------------------------------------------------------------------
-- Experiments 
class Executable2 a b where           
  type Inputs a
  type Outputs b 
  execute2 :: Function a b -> a -> ArBB (Outputs b) 
                                          
instance (Num b, Embeddable b , ArBBIO a,  
        --  ArBBIO (DVector () b), 
          IsScalar b, V.Storable b) => Executable2 a (DVector () b) where  
  type Inputs a = a 
  type Outputs (DVector () b) = b  
  execute2 (Function fn) inputs  = 
    do 
      (m,_) <- get 
      case Map.lookup fn m of 
        Nothing -> error "execute: Invalid function" 
...
instance (Embeddable (DVector t b) , ArBBIO a,  
          ArBBIO (DVector (():.t) b), 
          IsScalar b, V.Storable b) => Executable2 a (DVector (():.t) b) where  
  type Inputs a = a 
  type Outputs (DVector (():.t) b) = DVector (():.t) b  
  execute2 (Function fn) inputs  = 
    do 
      (m,_) <- get 
      case Map.lookup fn m of 
  ...
  {- 
instance (ArBBIO a, 
          Num b, IsScalar b, V.Storable b) => Executable2 a b where 
  type Inputs a = a 
  type Outputs b = b 
  execute2 (Function fn) inputs  = 
    do 
